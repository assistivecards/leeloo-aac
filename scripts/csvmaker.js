...
function savePack(index){
  ...
  if(language == "en"){
    ...
    let exists = fs.existsSync("../data/csv/"+language+"/interface.csv");
    if(exists && checkExist){
      ...
      UITranslation(language).then(res => {
        ...
        keys.forEach((trans, i) => {
          let line = `"${values[i]}", "${res[i]}"`;
          translationArr.push(line);
        });



        fs.mkdirSync("../data/csv/"+language+"/", {recursive: true});
        fs.writeFileSync("../data/csv/"+language+"/interface.csv", translationArr.join("\n"), {encoding: "utf8"});
